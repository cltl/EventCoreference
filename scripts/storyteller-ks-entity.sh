...
java -Xmx4000m -cp $LIB/EventCoreference-v3.0-jar-with-dependencies.jar eu.newsreader.eventcoreference.storyline.TrigToJsonStoryPerspectives --climax-level $CLIMAX   --ks-limit 1000 --story-limit 1000 --actors any $MERGE --time $TIME --action-ont $ONT --action-sim $ONTGRAN --actor-cnt $ACTOR --actor-intersect $INTERSECT --topic-level $TOPIC --ft "../../data/poll.data"  --blacklist "../../data/blacklist.txt" --eurovoc "$RESOURCES/mapping_eurovoc_skos.csv.gz" --eurovoc-blacklist "../../data/eurovoc-blacklist.txt" --project wikinews --action-schema "$ACTIONSCHEMA"  --service https://knowledgestore2.fbk.eu --entity $ENTITY --ks "nwr/wikinews-new" --ks-user wikinews --ks-pass wikinews 

# Command line example
#./storyteller-ks-entity.sh --merge W ili 5 5 2 50 50 "fn;eso" "Cameron"
