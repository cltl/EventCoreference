...
/*                try {
        ...
        SortedSet<PhraseCount> sortPhraseCounts (ArrayList<PhraseCount> pcounts) {
    ...
    static public void main (String[] args) {
       ...
       // ArrayList<ReferenceData> mergedData = merge(data, threshold);
        ...
        try {
          //  saveCsv(filePath+".ref", mergedData, data.size());
            saveDic(filePath+".dic", dictionary, data.size());
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    static ReferenceData csvToData (String line) {
        ...
        if (fields.length==8) {
           // System.out.println("fields.length = " + fields.length);
            ...
            if (!labelsString.isEmpty()) {
                ...
                referenceData.addLabels(labels);
            }
            ...
            try {
                ...
                while (in.ready()&&(inputLine = in.readLine()) != null) {
                    ...
                    if (inputLine.trim().length()>0) {
                        ReferenceData referenceData = csvToData(inputLine.trim());
    ...
    static void saveDic(String filePath, HashMap<String, ReferenceData> dic, Integer incidents) throws IOException {
        OutputStream fos = new FileOutputStream(filePath);
        String str = new File (filePath).getName()+"\n";
        str += "Total nr of events\t"+incidents+"\n";
